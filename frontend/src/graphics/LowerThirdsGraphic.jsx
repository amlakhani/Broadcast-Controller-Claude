...
export default function LowerThirdsGraphic({ socket, windowMode }) {
    ...
    const animateOut = contextSafe(() => {
        ...
        activeTimelineRef.current = tl;
        applyAnimationOut(tl, containerRef.current, currentAnimationRef.current, true, els);
    ...
    const panelFrameStyle = {
        width: isFullBand ? '100vw' : `${safePanelWidth}px`,
        maxWidth: isFullBand ? '100vw' : '1920px',
    ...
    const panelSurfaceStyle = {
        position: 'absolute',
        inset: 0,
        background: isUnderline ? 'transparent' : getThemeBackground(data?.bgStyle, panelAlpha),
        border: isUnderline ? 'none' : `${Number(design.borderWidth) || 0}px solid rgba(${borderRgb.r}, ${borderRgb.g}, ${borderRgb.b}, ${Math.max(panelAlpha * 0.6, 0.12)})`,
        ...
        WebkitBackdropFilter: isUnderline ? 'none' : 'blur(20px)',
    ...
    const positionStyle = {
        ...
        transform: isFullBand ? 'none' : `translate(${-normalizedX}%, ${normalizedY}%)`,
        ...
        <div ref={positionRef} style={positionStyle} className={`absolute flex items-center ${windowMode === 'stage' ? 'hidden' : ''}`}>
            <div id="lower-third" ref={containerRef} style={containerStyle} className={`flex items-center opacity-0 ${logoOnRight ? 'flex-row-reverse' : ''}`}>
                ...
                <div ref={panelRef} style={{ ...panelFrameStyle, padding: panelPadding }} className="relative overflow-hidden will-change-transform">
                    ...
                    {design.shapeStyle === 'ribbon' && <div className="absolute left-0 top-0 bottom-0 w-8" style={{ backgroundColor: design.accentColor }} />}
                    ...
                    {design.shapeStyle === 'full-band' && <div className="absolute top-0 left-0 right-0 h-1.5" style={{ backgroundColor: design.accentColor }} />}
                    ...
                    <div ref={ltSeparatorRef} className={`h-0.5 my-4 rounded-full w-full ${hasSubtitle2 ? '' : 'hidden'}`} style={{ backgroundColor: rgba(design.accentColor, 0.55) }}></div>
                    ...
                    {isUnderline && <div className="absolute left-0 right-0 bottom-0 h-1.5 rounded-full" style={{ backgroundColor: design.accentColor }} />}
                </div>
            </div>
        </div>
    );
